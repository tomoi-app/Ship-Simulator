...
const VOYAGE_LOCATIONS = {
  test_flip: { 
    name: "左右反転テスト", 
    lat: 35.308, lon: 139.695, // 第二海堡の真西
    heading: 0                 // 真北（000度）を向く
  },
  uraga: { 
    name: "浦賀水道", 
...
startLocation?.addEventListener('change', (e) => {
  ...
  } else if (val === 'test_flip') {
    // ★左右反転テスト用に追加
    goalLocation.disabled = false;
    goalLocation.style.color = "white";
    goalLocation.innerHTML = `<option value="uraga">テスト開始</option>`;
  } else {
    goalLocation.disabled = true;
    goalLocation.style.color = "gray";
    goalLocation.innerHTML = `<option value="">スタート位置を選んでください</option>`;
...
startVoyageBtn?.addEventListener('click', () => {
  ...
  // まず FREE-1 ミッションとして開始
  startM('FREE-1');

  // 自船（P）の座標と向きをセット
  P.posX = startPos.x;
  P.posZ = startPos.z;
  P.heading = startData.heading * (Math.PI / 180);
  P.speed = 0;
  P.engineOrder = 0;
  updateTelegraph(0);
  ...
  console.log(`航海開始: ${startData.name} -> ${VOYAGE_LOCATIONS[goalKey]?.name || goalKey}`);
});
