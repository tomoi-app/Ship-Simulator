...
function drawNeedle(ctx, val, minVal, maxVal, isRudder=false) {
    ...
    if (isRudder) {
        ...
        const headW = 3.5; const headL = 10;
        const normAngle = angle + Math.PI/2;
        const normCos = Math.cos(normAngle); const normSin = Math.sin(normAngle);
        
        ctx.lineTo(cx + cosA * (needleR - headL) + normCos * headW, cy + sinA * (needleR - headL) + normSin * headW);
        ctx.lineTo(cx + cosA * needleR, cy + sinA * needleR);
        ctx.lineTo(cx + cosA * (needleR - headL) - normCos * headW, cy + sinA * (needleR - headL) - normSin * headW);
        ctx.closePath();
        ctx.fillStyle = '#fff'; ctx.fill();
...
export function updateDashboard(P) {
    ...
    drawNeedle(ctx, rotDegMin, -30, 30, true);
    ...
    // 【修正】船首方位(Heading)を差し引いて「相対風向」を計算する
